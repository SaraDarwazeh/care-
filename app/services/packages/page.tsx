  ...
  useEffect(() => {
    ...
    listPackages()
      ...
      .catch((err) => {
        ...
        if (active) setError("Unable to load packages right now.");
      })
      .finally(() => {
    ...
    <PlatformShell mode="service">
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <section className="overflow-hidden rounded-[2rem] border border-sky-100 bg-white p-8 shadow-sm sm:p-10">
          <h1 className="text-3xl font-extrabold text-slate-900">Care Packages</h1>
          <p className="mt-3 text-slate-600">
            Browse our care packages to find the right plan for recovery, elderly support, or long-term care.
          </p>
        </section>
        ...
        {loading ? (
          ...
          <section className="mt-8 rounded-2xl border border-slate-100 bg-white p-8 text-center text-sm text-slate-500">
            No active care packages yet. Check back soon.
          </section>
        ) : (
          <section className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {packages.map((pkg) => (
