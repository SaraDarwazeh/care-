...
export default function NurseStorePage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-deep to-brand p-8 text-white shadow-lg flex justify-between items-center">
        <div className="relative z-10">
          <p className="text-sm font-bold text-brand-soft uppercase tracking-widest">{t("kicker")}</p>
          <h1 className="mt-2 text-3xl font-extrabold tracking-tight">{t("title")}</h1>
          <p className="mt-3 text-brand-soft max-w-md">{t("subtitle")}</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {products.map(item => {
              ...
              return (
                <div key={item.id} className="flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm hover:border-brand-soft hover:shadow-lg transition group">
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <div className="mb-2 flex items-start justify-between gap-2">
                      ...
                      <p className="font-bold text-brand-deep shrink-0">{fmtCurrency(item.price, locale)}</p>
                    </div>
                    ...
                    <div className="mt-auto pt-4 border-t border-slate-100">
                      {qty > 0 ? (
                        <div className="flex items-center justify-between bg-brand-soft/30 rounded-xl p-1">
                          <button onClick={() => removeFromCart(item.id)} className="p-2 text-brand hover:bg-brand-soft/50 rounded-xl transition">
                            <Minus className="h-4 w-4" />
                          </button>
                          <span className="font-bold text-brand-deep">{qty}</span>
                          <button onClick={() => addToCart(item.id)} className="p-2 text-brand hover:bg-brand-soft/50 rounded-xl transition">
                            <Plus className="h-4 w-4" />
                          </button>
                        </div>
                      ) : (
                        <button onClick={() => addToCart(item.id)}
                          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-slate-800 text-white font-bold text-sm hover:bg-brand-hover transition">
      ...
      {totalItems > 0 && (
        <div className="fixed bottom-6 inset-x-0 mx-auto max-w-md bg-slate-800 text-white p-4 rounded-3xl shadow-2xl flex items-center justify-between border border-slate-700 z-50 animate-in slide-in-from-bottom-10">
          ...
          <Link href="/patient/cart" className="bg-brand hover:bg-brand-hover transition px-6 py-2.5 rounded-2xl font-bold text-sm shadow-md">
            {t("checkout")}
          </Link>
        </div>
      )}
