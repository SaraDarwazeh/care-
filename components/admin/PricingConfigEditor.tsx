    ...
    <div className="rounded-3xl bg-white border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Add-on catalog, overnight surcharges, and tax rate. Edits apply to all new bookings.
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-6 p-6">
        ...
        <section className="grid gap-4 sm:grid-cols-3">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                min="0"
                step="0.5"
                value={config.overnightSurchargeAccepted}
                onChange={(e) =>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                min="0"
                step="0.5"
                value={config.overnightSurchargeRegular}
                onChange={(e) =>
          ...
          <div>
            <label className="mb-1 block text-sm font-bold text-slate-700">Tax rate</label>
            <div className="relative">
              <input
                type="number"
                min="0"
            ...
            <p className="mt-1 text-xs text-slate-500">Applied to subtotal. 0.05 = 5%.</p>
          </div>
        </section>

        <section>
          <label className="mb-1 block text-sm font-bold text-slate-700">
            Billed hours per shift block
          </label>
          <input
            type="number"
            min="1"
            value={config.shiftBilledHours}
            onChange={(e) =>
              setConfig({ ...config, shiftBilledHours: Math.max(1, Number(e.target.value) || 1) })
            }
            className="w-32 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm"
          />
          <p className="mt-1 text-xs text-slate-500">
            Used when computing shift base price = nurse hourly × billed hours.
          </p>
        </section>
